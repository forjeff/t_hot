...
<script type="text/javascript">
    $(document).ready(function(){
        GetData("A2");
        WriteSearchRecord(<?php echo "\"".$user_id."\""; ?>, $("#keyword").val(), $("#accident_type").val());
    });

    function GetData(accident_type){
        $.ajax({
            type: "POST",
            url: "Member_handler.php",
            dataType: "json",
            data: {fun_name: "SearchAccidentRoad", type: accident_type},
    ...
    function WriteSearchRecord(user_id, keyword, type){
        $.ajax({
            ...
            url: "Member_handler.php",
            dataType: "json",
            data: {fun_name: "SaveSearchAccidentRoadRecord", user_id: user_id, keyword: keyword, type: type},
            success: function(response){
...
<div id="floating-panel" style="background-color: rgba(60, 60, 60, 0.7)">
    <select id="accident_type">
        <?php
            $member = new Member_function();
            $type_list_data = $member->getSearchTypeList('交通事故');
